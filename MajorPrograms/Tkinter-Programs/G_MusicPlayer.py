...
class G_player:
    ...
    def mute_song(self):
        if self.muted:
            mixer.music.set_volume(0.7)  # Set the volume to a default level (70%)
            self.mute_button.config(image=self.unmute_img)  # Use instance variable self.unmute_img
            self.volume_scale.set(70)  # Reflect this in the volume scale
            self.muted = False
        else:
            mixer.music.set_volume(0)  # Mute the music
            self.mute_button.config(image=self.mute_img)  # Use instance variable self.mute_img
            self.volume_scale.set(0)  # Reflect this in the volume scale
            self.muted = True
